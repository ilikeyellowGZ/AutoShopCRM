...
export function VehicleGallery({ vehicle }: { vehicle: Vehicle }) {
  ...
  const move = (offset: number) => { setImageUnavailable(false); setDirection(offset > 0 ? "forward" : "back"); setActiveIndex((index) => (index + offset + images.length) % images.length); };
  ...
  return <section className="vehicle-gallery" aria-label="Vehicle gallery" tabIndex={0} onKeyDown={onKeyDown} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
    <div className="vehicle-gallery-stage">
      <button type="button" className="vehicle-gallery-image-button" onClick={() => setViewerOpen(true)} aria-label={`Open full screen image: ${active.alt}`}>
        {imageUnavailable ? <span className="vehicle-gallery-placeholder" role="img" aria-label={`${active.alt} unavailable`}>Vehicle image pending</span> : <img key={active.id} src={active.src} alt={active.alt} data-direction={direction} loading="eager" decoding="async" fetchPriority="high" onError={() => setImageUnavailable(true)} />}
      </button>
    ...
    <div className="vehicle-gallery-thumbnails" aria-label="Gallery thumbnails">{images.map((image, index) => <button type="button" key={image.id} className="vehicle-gallery-thumbnail" aria-current={activeIndex === index ? "true" : undefined} aria-label={`Show ${image.label}`} onClick={() => { setImageUnavailable(false); setDirection(index > activeIndex ? "forward" : "back"); setActiveIndex(index); }}><img src={image.src} alt="" loading="lazy" decoding="async" onError={(event) => { event.currentTarget.style.visibility = "hidden"; }} /><span>{index + 1}</span></button>)}</div>
