...
type InventoryPageProps = { state: DemoState; repository: DemoRepository; initialSubview?: string; onNavigate?: (subview: string, recordId?: string) => void; onCreateDeal?: (vehicleId: string) => void; viewPreferences?: ViewPreferences["inventory"]; onViewPreferencesChange?: (patch: Partial<ViewPreferences["inventory"]>) => void; canWrite?: boolean; canCreateDeal?: boolean; allowedBranches?: readonly string[]; draftScopeKey?: string; canViewCost?: boolean; canViewFinance?: boolean; canViewSalesInsights?: boolean; canViewAudit?: boolean };

function InventoryCardCover({ vehicle }: { vehicle: Vehicle }) {
  const [unavailable, setUnavailable] = useState(false);
  const cover = vehicle.gallery.images[0];
  const label = `${vehicle.year} ${vehicle.make} ${vehicle.model} ${vehicle.derivative} image unavailable`;

  if (!cover || unavailable) return <span role="img" aria-label={label}>{cover ? "Vehicle image unavailable" : "Vehicle image pending"}</span>;
  return <img src={cover.src} alt="" loading="lazy" decoding="async" onError={() => setUnavailable(true)} />;
}

export function InventoryPage({ state, repository, initialSubview = "list", onNavigate, onCreateDeal, viewPreferences, onViewPreferencesChange, canWrite = true, canCreateDeal = true, allowedBranches, draftScopeKey, canViewCost = true, canViewFinance = true, canViewSalesInsights = true, canViewAudit = true }: InventoryPageProps) {
  ...
  if (selected && subview === "detail") return <div data-testid="inventory-route" data-mount-identity={mountIdentity}><VehicleDetail vehicle={selected} state={state} repository={repository} onBack={() => navigate("list")} onCreateDeal={() => onCreateDeal?.(selected.id)} canWrite={canWrite} canCreateDeal={canCreateDeal} canViewCost={canViewCost} canViewFinance={canViewFinance} canViewSalesInsights={canViewSalesInsights} canViewAudit={canViewAudit} /></div>;
  if (subview === "intake" && canWrite) return <div data-testid="inventory-route" data-mount-identity={mountIdentity}><VehicleIntake key={draftScopeKey ?? "standalone"} state={state} repository={repository} allowedBranches={allowedBranches} draftScopeKey={draftScopeKey} onComplete={(id) => { setSelectedId(id); navigate("detail", id); }} onCancel={() => navigate("list")} /></div>;
  ...
  return <div data-testid="inventory-route" data-mount-identity={mountIdentity}><section className="inventory-page">
    <header className="inventory-heading"><div><p className="inventory-eyebrow">Vehicles</p><h1>Vehicle inventory</h1><p>{state.vehicles.length} connected vehicle records, maintained in this local demo workspace.</p></div>{canWrite ? <Button onClick={() => navigate("intake")}>Add vehicle</Button> : null}</header>
    <div className="inventory-metrics" data-tour="inventory-metrics"><MetricBlock label="On hand" value={state.vehicles.length} detail="Connected records" /><MetricBlock label="Available" value={available} detail="Ready to sell" /><MetricBlock label="Retail value" value={currency.format(state.vehicles.reduce((total, vehicle) => total + vehicle.price, 0))} detail="Current inventory" /></div>
