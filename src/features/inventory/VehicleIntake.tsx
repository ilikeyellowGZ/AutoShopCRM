...
export function VehicleIntake({ state, repository, onComplete, onCancel }: { state: DemoState; repository: DemoRepository; onComplete?: (vehicleId: string) => void; onCancel?: () => void }) {
  const draft = state.drafts.vehicleIntake?.values as Partial<IntakeValues> | undefined;
  const [step, setStep] = useState<1 | 2 | 3 | 4>(state.drafts.vehicleIntake?.step ?? 1);
  const [values, setValues] = useState<IntakeValues>({ ...defaults, ...draft });
  const [errors, setErrors] = useState<VehicleIntakeErrors>({});
  const save = (next: IntakeValues, nextStep = step) => { setValues(next); repository.updateDraft("vehicleIntake", { step: nextStep, values: next }); };
  const change = <K extends keyof IntakeValues>(key: K, value: IntakeValues[K]) => save({ ...values, [key]: value });
  const validation = useMemo(() => validateVehicleIntake(values, state), [state, values]);
  const advance = () => { const nextErrors = validateVehicleIntake(values, state); if (step === 1 && (nextErrors.vin || nextErrors.stockId)) { setErrors(nextErrors); return; } if (step === 2 && (nextErrors.year || nextErrors.make || nextErrors.model || nextErrors.price || nextErrors.bodyType || nextErrors.fuel || nextErrors.transmission || nextErrors.engine || nextErrors.registration)) { setErrors(nextErrors); return; } setErrors({}); const next = Math.min(4, step + 1) as 1 | 2 | 3 | 4; setStep(next); save(values, next); };
  const submit = () => { const nextErrors = validateVehicleIntake(values, state); if (Object.keys(nextErrors).length) { setErrors(nextErrors); return; } const id = `vehicle-${String(state.vehicles.length + 1).padStart(2, "0")}`; const derivative = values.derivative.trim() || "Standard"; const exterior = values.exterior.trim() || "Unspecified"; const vehicleName = `${values.year} ${values.make} ${values.model} ${derivative}`.trim(); const vehicle: Vehicle = { id, ...values, derivative, exterior, engine: values.engine.trim(), registration: values.registration.trim(), vin: values.vin.trim().toUpperCase(), stockId: values.stockId.trim().toUpperCase(), purchasePrice: Math.round(values.price * .82), daysInStock: 0, gallery: createVehicleGallery(slugify(vehicleName), vehicleName) }; repository.addVehicle(vehicle); repository.updateDraft("vehicleIntake", null); onComplete?.(id); };
  return <section className="vehicle-intake">
    <header className="vehicle-intake-heading"><div><p className="inventory-eyebrow">Inventory</p><h1>Vehicle intake</h1><p>Capture a vehicle record before it moves into the operational inventory.</p></div>{onCancel ? <Button variant="quiet" onClick={onCancel}>Cancel</Button> : null}</header>
    ...
    <div className="intake-content">
      ...
      {step === 1 ? <div className="intake-fields"><Field label="VIN" value={values.vin} error={errors.vin} onChange={(event) => change("vin", event.target.value)} /><Field label="Stock ID" value={values.stockId} error={errors.stockId} onChange={(event) => change("stockId", event.target.value)} /></div> : null}
      {step === 2 ? <div className="intake-fields intake-fields--three">
        ...
        <SelectField label="Branch" value={values.branch} onChange={(event) => change("branch", event.target.value)}>{demoBranches.map((branch) => <option key={branch}>{branch}</option>)}</SelectField>
      </div> : null}
