...
export function VehicleDetail({ vehicle, state, repository, onBack, onCreateDeal }: { vehicle: Vehicle; state: DemoState; repository: DemoRepository; onBack: () => void; onCreateDeal?: () => void }) {
  ...
  const documents = [...(vehicle.gallery.images.length ? [{ label: `Gallery manifest (${vehicle.gallery.images.length} images)`, status: "Ready" }] : []), ...service.map((job) => ({ label: `Service record: ${job.id}`, status: job.status })), ...(finance ? [{ label: "Finance worksheet", status: "Draft" }] : []), ...(deal ? [{ label: `Deal contract: ${deal.id}`, status: deal.status }] : [])];
