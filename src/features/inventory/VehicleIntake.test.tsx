...
it("completes four-step intake, persists the vehicle, gallery paths, and audit", async () => {
  ...
  await user.clear(screen.getByLabelText("Make")); await user.type(screen.getByLabelText("Make"), "Honda"); await user.clear(screen.getByLabelText("Model")); await user.type(screen.getByLabelText("Model"), "Civic"); await user.clear(screen.getByLabelText("Price (ZAR)")); await user.type(screen.getByLabelText("Price (ZAR)"), "1000000");
  await user.selectOptions(screen.getByLabelText("Body type"), "Hatchback");
  await user.selectOptions(screen.getByLabelText("Fuel"), "Hybrid");
  await user.selectOptions(screen.getByLabelText("Transmission"), "e-CVT");
  await user.clear(screen.getByLabelText("Engine")); await user.type(screen.getByLabelText("Engine"), "2.0L hybrid");
