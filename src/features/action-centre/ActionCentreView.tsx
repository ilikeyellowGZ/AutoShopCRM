...
function NotificationCentre({ state, repository, onNavigate, canMarkNotifications }: NotificationCentreProps) {
  ...
  return <section className="action-centre" aria-labelledby="notifications-title">
    ...
    <ul className="action-centre-list">{visible.length ? visible.map((notification) => {
      const target = targetForRelatedId(state, notification.relatedId);
      return <li key={notification.id}>
        <div>
          <StatusPill tone={notification.tone}>{notification.read ? "Read" : "Unread"}</StatusPill>
...
export function ActionCentreView({ state, repository, view = "actions", clock = demoClock, onNavigate, canManageTasks = true, canMarkNotifications = true }: ActionCentreViewProps) {
  if (view === "notifications") return <NotificationCentre state={state} repository={repository} onNavigate={onNavigate} canMarkNotifications={canMarkNotifications} />;
