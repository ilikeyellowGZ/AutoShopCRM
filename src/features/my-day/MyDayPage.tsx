...
function TaskRow({ task, onComplete, onReschedule, onNavigate, canManage }: { task: TaskItem; onComplete: () => void; onReschedule: () => void; onNavigate: () => void; canManage: boolean }) {
  return <li className="my-day-task">
    ...
    <div className="my-day-task-actions"><time dateTime={task.dueAt}>{timeFormat.format(new Date(task.dueAt))}</time>{canManage ? <><Button variant="secondary" onClick={onReschedule}>Reschedule</Button><Button onClick={onComplete}>Complete</Button></> : null}</div>
  </li>;
}
