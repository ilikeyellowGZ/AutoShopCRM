...
type SelectFieldProps = SharedProps & SelectHTMLAttributes<HTMLSelectElement>;
...
export function SelectField({ label, help, error, className = "", id, "aria-describedby": describedBy, children, ...props }: SelectFieldProps) {
  const generatedId = useId();
  const inputId = id ?? generatedId;
  const messageId = help || error ? `${inputId}-message` : undefined;
  return <label className={`field ${className}`.trim()} htmlFor={inputId}><span className="field-label">{label}</span><select id={inputId} className="field-control" aria-invalid={Boolean(error) || undefined} aria-describedby={[describedBy, messageId].filter(Boolean).join(" ") || undefined} {...props}>{children}</select><FieldMessage id={messageId ?? ""} help={help} error={error} /></label>;
}
