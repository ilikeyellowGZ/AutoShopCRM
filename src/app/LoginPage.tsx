...
import { DEMO_ACCESS_CODE, DEMO_TUTORIAL_ACCESS_CODE, authenticateDemoAccount, demoAccounts, type DemoAccount, type DemoExperienceLevel } from "./access";

export function LoginPage({ onSignIn }: { onSignIn: (account: DemoAccount, experience: DemoExperienceLevel) => void }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    onSignIn(result.account, result.experience);
  };
  ...
  return <main className="demo-login">
    ...
    <section className="demo-login-panel" aria-label="Demo sign-in">
      <form className="demo-login-form" onSubmit={submit} noValidate>
        ...
        <p className="demo-code-hint" id="demo-code-hint">First time here? Use <code>{DEMO_TUTORIAL_ACCESS_CODE}</code> for a guided tour of your role. Been here before? Use <code>{DEMO_ACCESS_CODE}</code> to skip straight in.</p>
      ...
      <div className="demo-account-directory">
        ...
        <ul>{demoAccounts.map((account) => <li key={account.id}><button type="button" style={{ "--role-color": account.color } as { [key: string]: string }} className="demo-account-card" aria-label={`Use ${account.email}`} onClick={() => { setEmail(account.email); setError(""); }}><strong>{account.title}</strong><span>{account.email}</span><small>{account.allowedBranches.length === 1 ? account.homeBranch : `${account.allowedBranches.length} branches`}</small></button></li>)}</ul>
      </div>
    </section>
  </main>;
}
